...
public class MyStartup implements IStartup {
	...
	public void earlyStartup() {
		Display.getDefault().asyncExec(new Runnable() {
			public void run() {
				...
				MyHandler.clipboard = new Clipboard(Activator.getDefault().getWorkbench().getDisplay());
	...
	private void deactivated(IWorkbenchPart part) {
		...
		if (cmdService == null) {
			cmdService = (ICommandService) part.getSite().getService(ICommandService.class);
			copyCommand = cmdService.getCommand("CopyCutCurrentLine.MyCopyCommand");
			cutCommand = cmdService.getCommand("CopyCutCurrentLine.MyCutCommand");
			pasteCommand = cmdService.getCommand("CopyCutCurrentLine.MyPasteCommand");
		}
